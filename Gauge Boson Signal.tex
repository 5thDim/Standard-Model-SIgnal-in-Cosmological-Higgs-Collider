...
\title{\Large\textbf{Standard Model Signals in the Cosmological Higgs Collider}\\[2mm]}
...
The Higgs bulk-to-boundary propagator is massless:
\begin{align}
    G_\pm(k; \tau) = \frac{H^2}{2k^3} (1 \mp ik\tau) e^{\pm ik\tau}
\end{align}

The Higgs--gauge couplings arise from the original SM Lagrangian:
\begin{align}
    \frac{\sqrt{-g}}{2}|\mathrm{D}_\mu \mb H |^2 \supset \frac{ a(\tau)^2}{4} g^2 (h_0 + \de h)^2 \left( W^+_\mu W^{-\mu} + \frac{1}{2c_W^2} Z^\mu Z_\mu \right)
\end{align}

The gauge-boson masses induced by infrared Higgs-loop corrections are:
\begin{align}
    m_W^2 = \frac{3 g^2 H^4}{8 \pi^2 m_H^2}, \quad m_Z^2 = \frac{m^2_W}{c^2_W}
\end{align}

The mode functions of a massive gauge boson are given by \cite{qin_helical_2023}:
\begin{align}
...
\end{align}
where $\tilde\nu=\sqrt{(m/H)^2-1/4}$. The Whittaker W function at zero chemical potential can be transformed into a Hankel function:
\begin{align}
...
Thus, the greater propagator, or $+-$ propagator, for each polarization is:
\begin{align}
...
For reference, the scalar propagator with mass index $i\tilde\nu$ is:
\begin{align}
    D^{(\text{scalar})}_> = \frac{\pi }{4 } e^{-\pi \widetilde{\nu}}\left(\tau_{1} \tau_{2}\right)^{3 / 2} \mathrm{H}_{\mathrm{i} \widetilde{\nu}}^{(1)}\left(-k \tau_{1}\right) \mathrm{H}_{-\mathrm{i} \widetilde{\nu}}^{(2)}\left(-k \tau_{2}\right)
\end{align}
and the nonlocal part of its late-time expansion is:
\begin{align}
    [D_>]_{\text{NonL}} = [D_<]_{\text{NonL}} = \frac{(\tau_1 \tau_2)^{3/2}}{4\pi} \left[ \Gamma^2(-i\tn) \left( \frac{k^2\tau_1\tau_2}{4} \right)^{i\tn} +\text{c.c.} \right]
\end{align}

The vector propagators can be related to the scalar propagator as follows:
\begin{align}
...
\end{align}
The propagator for the temporal component of the longitudinal mode is suppressed by a factor of $(k\tau)^2$. It therefore contributes only at next-to-leading order and is neglected below.

The corresponding nonlocal parts are:
\begin{align}
...
We also define the propagators for the helicity eigenstates:
\begin{align}
...
For $\mb{k}=(0,0,k)$, the polarization vectors are:
\begin{align}
...
\end{align}
When we rotate $\mb{k}$, $e_{\mu,\mathbf{k}}^{(T)}$ remains invariant, while the other two types of polarization vectors transform as $e_{i,\mathbf{k}}' = R_{ij}e_{j,\mathbf{k}}$:
\begin{align}
...
The MB representation of the scalar propagator is:
\begin{align}
...
For simplicity, we define the following loop seeds:
\begin{align}
...
\end{align}
where $f_0(a,b)$, $f_c(a,b)$, and $f_{cc}(a,b)$ are combinations of Gamma functions arising from the loop integrals:
\begin{align}
...
For the bubble diagram, we have:
\begin{align}
...
The relevant loop integral is:
\begin{align}
    \mathcal{LOOP}_{ab} &= \int \frac{\di \mb{q}^3}{(2\pi)^3} \left[ D_{\mu\nu}(q,\tau_1,\tau_2) \right]_{ab} \left[ D^{\nu\mu}(|\mb{k}_s + \mb{q}|,\tau_2,\tau_1) \right]_{ba}
\end{align}

The cutting rule implies that the nonlocal signal is independent of the SK index:
\begin{align}
...
\end{align}
where $\mb K=\mb q+\mb k_s$. We therefore consider the different polarization combinations. Setting $\mb k_s=k_s(0,0,1)$ and $\mb q=q(\sin\theta\cos\phi,\sin\theta\sin\phi,\cos\theta)$, the polarization vectors become:
\begin{align}
...
The signal arises from the loop integral over the nonlocal part of each propagator. We calculate the first term as an example:
\begin{align}
    \mathcal{{LOOP}}_{LL}
...
\end{align}
where $x=-\ii c\tn$. Using the identity $\mb q \cdot \mb K=\frac{1}{2}(q^2+K^2-k_s^2)$, we can instead write it as:
\begin{align}
    \mathcal{{LOOP}}_{LL}
     &= \sum_{c=\pm} \frac{(-1/2+\ii c\tn)^4}{(\tilde\nu^2 + 1/4)^2} \frac{(\tau_1 \tau_2)^{1+2ic\tn}}{(4\pi)^2} \Gamma^4(-ic\tn) \cdot \frac{4^{-2\ii c\tn} k_s^{3+4ic\tn}}{(4\pi)^{3/2}} \nonumber \\
    &\times \frac{1}{4} \left(f_0(x-1,x+1) + f_0(x+1,x-1) + f_0(x+1,x+1) \right. \nonumber \\
    &\quad \left. +2f_0(x,x) - 2f_0(x,x+1) - 2f_0(x+1,x)  \right)
\end{align}
...
For the transverse mode, we similarly have:
\begin{align}
    \mathcal{{LOOP}}_{++}
...
Taking the vector structure into account gives:
\begin{align}
...
We note that the $--$ term equals the $++$ term.
We next consider the mixed terms, beginning with the $L+$ term:
\begin{align}
    \mathcal{{LOOP}}_{L+} &= \int \frac{\di \mb{q}^3}{(2\pi)^3} \left[ D^{(L)}(q,\tau_1,\tau_2) e^{(L)i}_{\mb q} e^{(L)j*}_{\mb q} \cdot D^{(+)}(K,\tau_2,\tau_1) e^{(+)j}_{\mb K} e^{(+)i*}_{\mb K} \right]_{\text{NonL}} \nonumber \\
    &= \sum_{c=\pm} \frac{(-1/2+\ii c\tn)^2}{(\tilde\nu^2 + 1/4)} \frac{(\tau_1 \tau_2)^{1+2ic\tn}}{(4\pi)^2} \Gamma^4(-ic\tn) \int \frac{\di \mb{q}^3}{(2\pi)^3}\left[  \left( \frac{q}{2} \right)^{2ic\tn} \left( \frac{K}{2} \right)^{2ic\tn} |e^{(L)}_{\mb q} \cdot e^{(+)*}_{\mb K}|^2 \right]
\end{align}
which gives:
\begin{align}
    \mathcal{{LOOP}}_{L+} &= \sum_{c=\pm} \frac{(-1/2+\ii c\tn)^2}{(\tilde\nu^2 + 1/4)} \frac{(\tau_1 \tau_2)^{1+2ic\tn}}{(4\pi)^2} \Gamma^4(-ic\tn) \cdot \frac{4^{-2\ii c\tn} k_s^{3+4ic\tn}}{(4\pi)^{3/2}} \nonumber \\
...
\end{align}
Evaluating the dot products, we find that $L-=L+=-L=+L$.
...
Thus, the full loop integral is:
\begin{align}
    \mathcal{LOOP} = 
    \sum_{\si_1,\si_2=L,\pm}
...
We now consider the contributions from the left and right subgraphs, namely, the time integrals.
...
Similarly, for the right subgraph:
\begin{align}
    \mathcal{T}^{(R)}_c = (g^2) \frac{H^2}{4k_3^3 k_4^3} \times k_{34}^{-2ic\tn} \cdot \mathcal{P}^{(1)}_c(\tn)
\end{align}

Thus, the total contribution of the bubble diagram is:
\begin{align}
...
A numerical result is shown below:
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figure/CBub.png}
    \caption{$C_{\text{Bub}}(\tn)$}
\end{figure}

\newpage
\section{Triangle Diagram}
One of the triangle diagrams is given by:
\begin{align}
...
First, we consider the loop integral:
\begin{align}
    \mathcal{LOOP} 
...
According to the cutting rule, this integral contributes to the nonlocal signal only when both $\mb q$ and $\mb k_s$ become soft.
\begin{align}
...
\begin{align}
    ...
    \end{pmatrix}.
\end{align}
Expanding the propagators in the helicity basis gives:
\begin{align}
...
\end{align}
where the contribution from each polarization combination is:
\begin{align}
...
First, we consider the $\lambda=L$ terms and evaluate the contractions with the other two propagators. We set
...
\end{align}
Thus, the nine contractions are:
\begin{align}
...
We can then evaluate the loop integrals for the terms involving $D^{(L)}(k_1)$ and extract the nonlocal signal:
\begin{align}
    \mathcal{LOOP}_{\sigma_1\sigma_2}^{\lambda} &= \int \frac{\di \mb{q}^3}{(2\pi)^3} 
    [D^{(\lambda)}(k_1,\tau_1,\tau_2)]_{ab} 
    D^{(\sigma_1)}(K,\tau_2,\tau_3) 
...
\end{align}
where $\beta^{\si L}$ is an additional factor for the longitudinal mode:
\begin{align}
    \beta^{\si L} = \begin{cases}
    \frac{(-1/2+\ii d\tn)^2}{\tn^2+1/4}, & \si=L, \\
    1, & \si=\pm.
    \end{cases}
\end{align}
...
We denote $z=\cos\theta$, $s_1=\sin\theta_1$, and $c_1=\cos\theta_1$. The nonvanishing parts are:
\begin{align}
...
We denote the dimensionless coefficients of the loop integrals by $\mathcal{I}_{\si_1\si_2}^{\lam}(-\ii d\tn)$ and define them through:
\begin{align}
    \mathcal{LOOP}_{\sigma_1\sigma_2}^{\lambda} 
    &= \left[D^{(\lambda)}(k_1,\tau_1,\tau_2)\right]_{ab}
    \cdot \sum_{d=\pm}
...
\begin{align}
    ...
    &=\frac{4^{-2\ii d\tn}k_s^{3+4\ii d\tn}}{(4\pi)^{3/2}}
...
\begin{align}
    \mathcal{I}_{LL}^{L}
    &= \frac{(-1/2+\ii d\tn)^4}{(\tn^2 + 1/4)^2} \cdot
    ...
    \mathcal{I}_{L+}^{L}
    =\mathcal{I}_{L-}^{L}
    &= \frac{(-1/2+\ii d\tn)^2}{(\tn^2 + 1/4)} \cdot \left\{ -\frac{s_1^2}{4}\left[\mathcal{J}_1(1,2)-\mathcal{J}_3(1,2)\right] \right. \nonumber \\
    &\quad \left.
    +\frac{c_1^2}{2}\left[\mathcal{J}_1(1,2)+\mathcal{J}_0(0,2)
    -\mathcal{J}_3(1,2)-\mathcal{J}_2(0,2)\right] \right\} , \\
    \mathcal{I}_{+L}^{L}
    =\mathcal{I}_{-L}^{L}
    &= \frac{(-1/2+\ii d\tn)^2}{(\tn^2 + 1/4)} \cdot \left\{ \frac{s_1^2}{4}\left[\mathcal{J}_1(1,2)+\mathcal{J}_0(0,2)
    -\mathcal{J}_3(1,2)-\mathcal{J}_2(0,2)\right] \right. \nonumber \\
    &\quad \left.
...
Thus, this part of the triangle loop integral can be written as:
\begin{align}
    \mathcal{LOOP}^{L} 
    &= \sum_{\si_1,\si_2=L,\pm} 
    ...
    \mathcal{I}^{L},
\end{align}
where $\mathcal{I}^L$ is defined as the sum of the nine contractions:
\begin{align}
    \mathcal{I}^{L}
...
We now repeat the same calculation for the $\lambda=\pm$ terms.
...
After taking the azimuthal average, we obtain the nonvanishing parts of the nine contractions:
\begin{align}
    \bar{\mathcal{E}}^+_{LL} 
    &= \frac{q+k_s z}{2K^2}
    \left[
...
Applying the same loop-seed map as above, we obtain the dimensionless coefficients:
\begin{align}
    {\mathcal{I}}^+_{LL} 
...
Thus, the sum of these nine contractions is:
\begin{align}
    \mathcal{I}^{+}
    &\equiv \sum_{\si_1,\si_2=L,\pm} \mathcal{I}^{+}_{\si_1\si_2}
\end{align}

For the $D^{(-)}(k_1)$ factors, the polarization contractions satisfy:
\begin{align}
...
Thus, the transverse part of the triangle loop integral is:
\begin{align}
    \mathcal{LOOP}^{\si}
    =
...
The total loop integral therefore becomes:
\begin{align}
    \mathcal{LOOP}
    =
    \sum_{\lambda=L,\pm}
...
We next evaluate the contributions from the left and right subgraphs:
\begin{align}
...
For $\lambda=L$, this integral is given by the tree-level seed of Ref.~\cite{qin_helical_2023}:
\begin{align}
    \mathcal{T}^{(L),L}_d 
    &= \left( g^2h_0\right)^2 
...
\end{align}
Here, $\mathcal{K}_{ab}^{p_1p_2} = \mathcal{I}^{(L)p_1p_2}_{ab}(1,1)$ are the vector seeds in the double-folded limit \cite{qin_helical_2023}. The opposite-sign seeds are:
\begin{align}
...
However, the double-folded limit is difficult to evaluate in this form.
We therefore use another method, namely, applying a differential operator to the scalar seed \cite{qin_closed-form_2023}.
The coefficient then decomposes into two parts, the first of which contains the delta-function contribution:
\begin{align}
    \text{Part 1 of}\;\; \mathcal{P}_d^{(2),L}(\tn) = \mathcal{P}_d^{(1)}(\tn) \cdot \frac{1}{\tn^2+1/4}
\end{align}

The second part arises from the scalar seeds:
\begin{align}
...
\end{align}
where $p_0=-\fr52+\ii d\tn$, and $\mathcal{M}_{ab}^{p_1p_2} = \mathcal{I}^{p_1p_2}_{ab}(1,1)$ are the scalar seeds defined in \cite{qin_closed-form_2023}. Since $p_0$ lies on the boundary of the seed convergence domain, the expressions are understood by analytic continuation from $p_0=-\fr52+\epsilon+\ii d\tn$, taking the finite part as $\epsilon\to0^+$.
The opposite-sign seeds are:
\begin{align}
...
For $\lambda=\pm$, this integral can also be expressed in terms of the tree-level seed of Ref.~\cite{qin_closed-form_2023}:
\begin{align}
    \mathcal{T}^{(L),\pm}_d 
    &= (g^2h_0)^2  
    \frac{H^2}{4k_1^3 k_2^3}
...
Similarly, for the right subgraph:
\begin{align}
    \mathcal{T}^{(R)}_d &= (g^2) \sum_c ic \int \frac{\di \tau_3}{|H \tau_3|^2} G_c(k_3;\tau_3) G_c(k_4;\tau_3) (-\tau_3)^{1+2\ii d\tn} \nonumber \\
    &= (g^2) \frac{H^2}{4k_3^3 k_4^3} \sum_{c=\pm} ic \int_{-\infty}^{0} \di \tau_3 (-\tau_3)^{-1+2\ii d\tn} (1 -ick_{34}\tau_3-k_3k_4\tau_3^2) e^{ick_{34}\tau_3} \nonumber \\
...
Thus, the contribution of one triangle diagram is:
\begin{align}
...
For the second triangle diagram:
\begin{align}
...
Thus, the total contribution of the triangle diagrams is:
\begin{align}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figure/CTri.png}
    \caption{$C_{\text{Tri}}(\tn)$}
\end{figure}

\newpage
\section{Box Diagram}
For the box diagram, we have:
\begin{align}
...
Keeping the latter two helicities explicit, as in the triangle calculation, we define
\begin{align}
...
We then evaluate the contributions from the left and right subgraphs. Both are identical to the left-subgraph contribution in the triangle calculation:
\begin{align}
    \mathcal{T}^{(L),\lam_1}_f 
    &= \left( ig^2h_0\right)^2 
    \frac{k_{12}^{-2\ii f\tn}}{4k_1^3 k_2^3}
    \cdot \mathcal{P}_f^{(2),\lam_1}(\tn) \\
    \mathcal{T}^{(R),\lam_2}_f 
    &= \left( ig^2h_0\right)^2 
    \frac{k_{34}^{-2\ii f\tn}}{4k_3^3 k_4^3}
    \cdot \mathcal{P}_f^{(2),\lam_2}(\tn)
\end{align}
Thus, the contribution of the box diagram is:
\begin{align}
    ...
    \frac{H^2 k_s^3}{16 k_1^3 k_2^3 k_3^3 k_4^3} 
    \left( \frac{k_s^2}{4k_{12}k_{34}}\right)^{2\ii\tn} 
    \times C^{\text{Box}}(\tn,\theta_1,\theta_3,\phi_3) + \text{c.c.}
\end{align}
where the magnitude is:
\begin{align}
    C^{\text{Box}}(\tn,\theta_1,\theta_3,\phi_3) = 
    \sum_{\lam_1,\lam_2=L,\pm}
    \frac{\Gamma^4(-\ii \tn)}{(4\pi)^{7/2}} \cdot
...
Since $\mathcal{P}^{(2),+}_+(\tn)=\mathcal{P}^{(2),-}_+(\tn)$, we define:
\begin{align}
    &\mathcal{I}^{LH} = \mc I^{L+} + \mc I^{L-} +\mc I^{+L} + \mc I^{-L} \\
    &\mathcal{I}^{HH} = \mc I^{++} + \mc I^{--} +\mc I^{+-} + \mc I^{-+}
\end{align}

The magnitude can then be written as:
\begin{align}
    C^{\text{Box}}(\tn,\theta_1,\theta_3,\phi_3) = 
    \frac{\Gamma^4(-\ii \tn)}{(4\pi)^{7/2}} \cdot
    \left\{
        [\mathcal{P}^{(2),L}_+]^2 \cdot  
        \mathcal{I}^{LL}
        + [\mathcal{P}^{(2),L}_+ \mc P^{(2),+}_+ ]\cdot  
        \mathcal{I}^{LH}
        + [\mathcal{P}^{(2),+}_+]^2 \cdot  
        \mathcal{I}^{HH}
    \right\}
\end{align}

Through a lengthy calculation, we obtain an analytic expression for $C^{\text{Box}}$, which we write as:
\begin{align}
    C^{\text{Box}}(\tn,\theta_1,\theta_3,\phi_3) &
    = A_1
    +A_2(\cos^2\theta_1 + \cos^2\theta_3)
    +A_3(\cos^2\theta_1 \cdot \cos^2\theta_3) \nonumber \\
    &+ B \sin(2\theta_1) \sin(2\theta_3) \cos \phi_3
    + D \sin^2\theta_1 \sin^2\theta_3 \cos 2\phi_3
\end{align}

Some numerical results are shown below:
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figure/CBoxA1.png}
    \caption{$A_1(\tn)$}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figure/CBoxA2.png}
    \caption{$A_2(\tn)$}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figure/CBoxA3.png}
    \caption{$A_3(\tn)$}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figure/CBoxB1.png}
    \caption{$B(\tn)$}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figure/CBoxD1.png}
    \caption{$D(\tn)$}
\end{figure}
